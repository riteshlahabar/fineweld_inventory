...
@section('title', __('partnership::contract.report'))
...
<div class="tab-pane fade show active" id="partyOpeningBalanceTab" role="tabpanel">
    ...
    <div class="row g-3">
        <form class="row g-3 needs-validation" id="reportForm" action="{{ route('partnership.report.contract.ajax') }}" enctype="multipart/form-data">
                        {{-- CSRF Protection --}}
                        @csrf
                        @method('POST')
                                            ...
                                            <table class="table table-bordered" id="recordsTable">
                                                <thead>
                                                    <tr class="text-uppercase">
                                                        ...
                                                        <th>{{ __('partnership::contract.total_items') }}</th>
                                                        <th>{{ __('partnership::contract.total_partners') }}</th>
                                                    </tr>
                                                </thead>
                                                <tbody></tbody>
                                            </table>
...
<script src="{{ versionedAsset('custom/js/modules/partnership/reports/contract.js') }}"></script>
@endpush
