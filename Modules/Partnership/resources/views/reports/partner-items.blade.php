...
@section('title', __('partnership::partner.current_items'))
...
<div class="tab-pane fade show active" id="partyOpeningBalanceTab" role="tabpanel">
    ...
    <div class="row g-3">
        <form class="row g-3 needs-validation" id="reportForm" action="{{ route('partnership.report.partner-items.ajax') }}" enctype="multipart/form-data">
                        {{-- CSRF Protection --}}
                        @csrf
                        @method('POST')
                                        ...
                                        <div class="col-md-6 mb-3">
                                            <div>
                                                <div class="form-check mt-2">
                                                    <input class="form-check-input" type="checkbox" id="show_active" name="show_active" value="1" checked>
                                                    <label class="form-check-label" for="show_active">{{ __('partnership::partner.show_currently_active_items_of_partner') }}</label>
                                                </div>
                                            </div>
                                        </div>
                                </div>
...
<script src="{{ versionedAsset('custom/js/modules/partnership/reports/partner-items.js') }}"></script>
@endpush
