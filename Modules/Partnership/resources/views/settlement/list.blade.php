@extends('layouts.app')
@section('title', __('partnership::partner.partner_settlement_list'))
        ...
        <div class="page-wrapper">
            <div class="page-content">
                    ...
                    <div class="card-header px-4 py-3 d-flex justify-content-between">
                        ...
                        <div>
                            <h5 class="mb-0 text-uppercase">{{ __('partnership::partner.partner_settlement_list') }}</h5>
                        </div>

                        @can('partner.contract.create')
                        <!-- Button pushed to the right side -->
                        <x-anchor-tag href="{{ route('partnership.partner.settlement.create') }}" text="{{ __('partnership::partner.create_settlement') }}" class="btn btn-primary px-5" />
                        @endcan
                    </div>
                    <div class="card-body">
                        <div class="row g-3">
                        ...
                        <form class="row g-3 needs-validation" id="datatableForm" action="{{ route('partnership.partner.settlement.delete') }}" enctype="multipart/form-data">
                            {{-- CSRF Protection --}}
                            @csrf
                            @method('POST')
                                ...
                                <table class="table table-striped table-bordered border w-100" id="datatable">
                                    <thead>
                                        <tr>
                                            ...
                                            <th>{{ __('app.date') }}</th>
                                            <th>{{ __('partnership::partner.settlement_code') }}</th>
...
<script src="{{ versionedAsset('custom/js/modules/partnership/settlement/settlement-list.js') }}"></script>
