<?php

return [

    'warehouse' => 'مستودع',
    'warehouses' => 'المستودعات',
    'list' => 'قائمة المستودعات',
    'create' => 'إنشاء مستودع',
    'add_warehouse' => 'إضافة مستودع',
    'details' => 'تفاصيل المستودع',
    'update' => 'تحديث المستودع',

    'warehouse_stock' => 'مخزون المستودع',
    'stock_transfer' => 'نقل المخزون',
    'stock_transfer_list' => 'قائمة نقل المخزون',
    'new_transfer' => 'نقل جديد',
    'from_warehouse' => 'من المستودع',
    'to_warehouse' => 'إلى المستودع',
    'qty_to_transfer' => 'الكمية المراد نقلها',
    'stock_transfer_details' => 'تفاصيل نقل المخزون',
    'stock_transfer_report' => 'تقرير نقل المخزون',
    'item_stock_transfer_report' => 'تقرير نقل مخزون العناصر',
    'transfer_code' => 'رمز النقل',
    'default' => 'المستودع الافتراضي',
    'allow_all_warehouses' => 'السماح بجميع المستودعات',

    'user_wise_warehouses_records' => 'إذا تركتها فارغة، ستظل السجلات تُعرض بناءً على المستودعات المخصصة للمستخدمين!',
    'allow_user_to_view_all_stock_transfer' => 'السماح للمستخدم بعرض جميع عمليات نقل المخزون التي أنشأها مستخدمون آخرون',
    // 2.3.2
    'stock_adjustment' => 'تسوية المخزون',
    'stock_adjustment_list' => 'قائمة تسوية المخزون',
    'stock_adjustment_details' => 'تفاصيل تسوية المخزون',
    'stock_adjustment_report' => 'تقرير تسوية المخزون',
    'item_stock_adjustment_report' => 'تقرير تسوية مخزون العناصر',
    'allow_user_to_view_all_stock_adjustment' => 'السماح للمستخدم بعرض جميع تسويات المخزون التي أنشأها مستخدمون آخرون',
    'new_adjustment' => 'تسوية جديدة',
    'adjustment_qty' => 'كمية التسوية',
    'adjustment_code' => 'رمز التسوية',
    'transfer' => 'نقل',
    'adjustment' => 'تسوية',
    'adjustment_type' => 'نوع التسوية',

];
