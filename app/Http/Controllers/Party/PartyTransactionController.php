...
class PartyTransactionController extends Controller
{
            ...
            if (! empty($record) && count($record) > 0) {
                ...
                foreach ($record as $recordId => $amount) {
                    ...
                    if (isPartnershipModuleActive()) {
                        // For Customer Sales
                        if ($party->party_type == 'customer') {
                            // We accepting only sale payment of customer not a sale return payment
                            \Modules\Partnership\Events\SalePaymentCreated::dispatch($mainModel);
                        }
                    }
     ...
     * @throws \Exception
     */
    public function validateModel($model, $partyId, $documentType, $amount)
    {
    ...
    public function getPartyPaymentHistoryData($partyId)
    {
        try {
