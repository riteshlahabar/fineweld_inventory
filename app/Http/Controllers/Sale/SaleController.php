...
class SaleController extends Controller
{
     ...
     * @return \Illuminate\View\View
     */
    public function print($invoiceFormat, $id, $isPdf = false): View
    {
     ...
     * */
    public function generatePdf($invoiceFormat, $id, $destination = 'D')
    {
        $random = uniqid();
    ...
    public function saveSaleItems($request)
    {
        ...
        for ($i = 0; $i < $itemsCount; $i++) {
            ...
            } elseif ($itemDetails->tracking_type == 'batch') {
                // Serial validate and insert records
                if ($itemQuantity > 0) {
                    /**
     ...
     */
    public function getSoldItemsData($partyId, $itemId = null)
    {
        try {
            $sales = Sale::with([
                'party',
            ...
            $partyName = $sales->first()->party->company_name ?? '';
    ...
    $partyQuery->where('company_name', 'LIKE', "%{$search}%");
});
            });
        ...
        $results = $invoices->map(function ($invoice) {
            return [
                ...
                'party_name' => optional($invoice->party)->company_name,
            ];
        });
