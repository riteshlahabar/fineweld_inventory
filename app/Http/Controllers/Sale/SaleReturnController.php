...
class SaleReturnController extends Controller
{
    ...
    public function store(SaleReturnRequest $request): JsonResponse
    {
        try {
            ...
            if (isPartnershipModuleActive()) {
                \Modules\Partnership\Events\SaleReturnCreated::dispatch($newRuturn);
                \Modules\Partnership\Events\SaleReturnPaymentCreated::dispatch($newRuturn);
            }
    ...
    public function datatableList(Request $request)
    {
        ...
        $data = SaleReturn::with('user', 'party')
            ...
            ->when(! auth()->user()->can('sale.return.can.view.other.users.sale.returns'), function ($query) {
