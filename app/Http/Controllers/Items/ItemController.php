...
class ItemController extends Controller
{
    ...
    public function store(ItemRequest $request)
    {
        try {
            ...
            } elseif ($request->tracking_type == 'batch') {
                // Serial validate and insert records
                if ($request->opening_quantity > 0) {
     ...
     * Search for Select2 Bar list
     * */
    public function getAjaxItemBatchSearchBarList()
    {
        $search = request('search');
     ...
     *
     * */
    public function getAjaxItemBatchTableRecords()
    {
        $search = request('search');
     ...
     * Search for Select2 Bar list
     * */
    public function getAjaxItemSerialSearchBarList()
    {
        $search = request('search');
        $itemId = request('item_id');
     ...
     * Search Bar list
     * */
    public function getAjaxItemSearchBarList()
    {
        $search = request('search');
    ...
    public function returnRequiredFormatData($itemMaster, $showWholesalePrice = false)
    {
     ...
     * Search Bar list
     * */
    public function getAjaxItemSerialIMEISearchBarList()
    {
    ...
    public function getAjaxItemBatchStockList($autocompleteWithBatchNo = false)
    {
        ...
        $response = $batchQuantity->map(function ($quantity) use ($itemData) {
            ...
            $availableStockWarehouseWise = ItemBatchQuantity::where('item_id', $quantity->item_id)
                ->where('warehouse_id', $quantity->warehouse_id)
                ->where('item_batch_master_id', $quantity->item_batch_master_id)
                ->sum('quantity');
            $availableStockWarehouseWise = $this->itemService->getQuantityInUnit($availableStockWarehouseWise, itemId: $quantity->item_id);
            ...
            return [
                ...
                'availableStock' => $availableStockWarehouseWise,
                'saleQuantity' => '',
