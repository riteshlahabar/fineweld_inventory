$(function() {
    ...
    function formAdjustIfSaveOperation(response){
        ...
        $.each(response.data, function(index, item) {
            totalProfit += parseFloat(item.amount);
            ...
            tr  +=`
                <tr>
                    ...
                    <td>${item.payment_direction}</td>
                    <td class='text-end ${item.color}' data-tableexport-celltype="number" >${_formatNumber(item.amount)}</td>
                </tr>
            `;
        });
        ...
        tr  +=`
            <tr class='fw-bold'>
                ...
                <td class='text-end ${parseFloat(response.total) < 0 ? "text-danger" : ""}' data-tableexport-celltype="number">${_formatNumber(response.total)}</td>
            </tr>
        `;
