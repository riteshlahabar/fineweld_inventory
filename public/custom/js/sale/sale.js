"use strict";
    ...
    function pageRedirect(formObject){
        ...
        if(formObject.response.id !== undefined){
            redirectTo = '/sale/invoice/details/'+formObject.response.id;
        }else{
            redirectTo = '/sale/invoice/list';
        }
        setTimeout(function() {
    ...
    itemSearchInputBoxId.on("keydown", function(event) {
        ...
        if(!partyId.val()){
            iziToast.info({title: 'Message', layout: 2, message: _lang.pleaseSelectParty});
   ...
   itemSearchInputBoxId.on('click', function() {
    ...
    function autoSetClassOfTableRow(trackingType, rowId){
        ...
        //Add tooltip to this input box by shwowing the message
        $(`input[name="discount[${rowId}]"]`).filter('[readonly]').addClass('cursor-not-allowed').attr('data-bs-toggle', 'tooltip').attr('data-bs-placement', 'top').attr('data-bs-original-title', _lang.discountNotAllowed);

        if (!allowUserToChangeSalePrice) {
            $(`input[name="sale_price[${rowId}]"]`).attr('readonly', true).addClass('cursor-not-allowed').attr('data-bs-toggle', 'tooltip').attr('data-bs-placement', 'top').attr('data-bs-original-title', _lang.salePriceNotAllowed);
        }
    ...
    */
    async function deletePaymentRequest(paymentId) {
        ...
        if (confirmed) {
            var url = baseURL + '/payment/sale-invoice/delete/';
    ...
    $(document).on('change', '#party_id', function() {
        var selectedData = partyId.select2('data')[0]; // `data()` returns an array, take the first item
