@extends('layouts.app')
@section('title', __('purchase.return.return'))
        ...
        <div class="page-wrapper">
            <div class="page-content">
                ...
                <div class="row">
                    <form class="g-3 needs-validation" id="invoiceForm" action="{{ ($return->operation == 'convert') ?  route('purchase.to.purchase.return.save') : route('purchase.return.update') }}" enctype="multipart/form-data">
                        {{-- CSRF Protection --}}
                        @csrf

                        {{-- Holds PUT method if Edit operation, else holds POST if Purchase converting to Purchase Return --}}
                        @if($return->operation == 'convert')
                            @method('POST')
                        @else
                            @method('PUT')
                        @endif
                        ...
                        {{-- Holds PUT method if Edit operation, else holds POST if Purchase converting to Purchase Return --}}
                        @if($return->operation == 'convert')
                            {{-- <input type="hidden" name="purchase_id" value="{{ $return->id }}"> --}}
                        @else
                        ...
                        <input type="hidden" id="operation" name="operation" value="{{ $return->operation }}">
                            ...
                            <div class="col-12 col-lg-12">
                                <div class="card">
                                    ...
                                    <div class="card-body p-4 row g-3">
                                            <div class="col-md-4">
                                                ...
                                                <div class="input-group">
                                                    <select class="form-select party-ajax" data-party-type='supplier' data-placeholder="Select Supplier" id="party_id" name="party_id">
                                                        <option value="{{ $return->party->id }}">{{ $party->company_name }}</option>
                                                    </select>
                                            ...
                                            <div class="col-md-4">
                                                <x-label for="return_date" name="{{ __('app.date') }}" />
                                                <div class="input-group mb-3">
                                                    <x-input type="text" additionalClasses="datepicker-edit" name="return_date" :required="true" value="{{ $return->formatted_return_date }}"/>
                                                    <span class="input-group-text" id="input-near-focus" role="button"><i class="fadeIn animated bx bx-calendar-alt"></i></span>
                                                </div>
                                            </div>

                                            <div class="col-md-4">
                                                <x-label for="purchase_code" name="{{ __('purchase.return.code') }}" />
                                                <div class="input-group mb-3">
                                                    <x-input type="text" name="prefix_code" :required="true" placeholder="Prefix Code" value="{{ $return->prefix_code }}"/>
                                                    <span class="input-group-text">#</span>
                                                    <x-input type="text" name="count_id" :required="true" placeholder="Serial Number" value="{{ $return->count_id }}"/>
                                                </div>
                                            </div>
                                            <div class="col-md-4">
                                                <x-label for="reference_no" name="{{ __('purchase.purchase_bill_number') }}" />
                                                <x-input type="text" name="reference_no" :required="false" placeholder="(Optional)" value="{{ $return->reference_no }}"/>
                                            </div>
                                            @if(app('company')['tax_type'] == 'gst')
                                            <div class="col-md-4">
                                                <x-label for="state_id" name="{{ __('app.state_of_supply') }}" />
                                                <x-dropdown-states selected="{{ $return->state_id }}" dropdownName='state_id'/>
                                            </div>
                                            @endif
                                            ...
                                            <div class="col-md-4">
                                                ...
                                                <div class="input-group mb-3">
                                                    <x-dropdown-currency selected="{{ $return->currency_id }}" name='invoice_currency_id'/>
                                                    <x-input type="text" name="exchange_rate" :required="false" additionalClasses='cu_numeric' value="{{ $return->exchange_rate }}"/>
                                                </div>
                                            </div>
                                            @endif
                                    ...
                                    <div class="card-body p-4 row g-3">
                                            ...
                                            <div class="col-md-8">
                                                ...
                                                <x-textarea name='note' value='{{ $return->note }}'/>
                                            </div>
                                                ...
                                                <table class="table mb-0 table-striped">
                                                   <tbody>
                                                      ...
                                                      <tr>
                                                         ...
                                                         <td>
                                                            <x-input type="text" additionalClasses="text-end paid_amount" readonly=true :required="false" placeholder="Paid Amount" value="{{ $formatNumber->formatWithPrecision($return->paid_amount) }}"/>
                                                        </td>
                                                      </tr>
                                                      <tr>
...
<script src="{{ versionedAsset('custom/js/purchase/purchase-return.js') }}"></script>
