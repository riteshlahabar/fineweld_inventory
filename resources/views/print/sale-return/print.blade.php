        ...
        <div class="invoice">
            <table class="header">
                <tr>
                    ...
                    <td class="bill-info">
                        <span class="bill-number">#{{ $sale->return_code }}</span><br>
                        <span class="cu-fs-16">{{ __('sale.return.date') }}: {{ $sale->formatted_return_date  }}</span><br>
                        <span class="cu-fs-16">{{ __('app.time') }}: {{ $sale->format_created_time }}</span><br>
                        @if($sale->reference_no)
                        <span class="cu-fs-16">{{ __('sale.sale_invoice_number') }}: {{ $sale->reference_no  }}</span><br>
                        @endif
                    </td>
                </tr>
            </table>
            <table class="addresses">
                <tr>
                    <td class="address">
                        <span class="fw-bold cu-fs-18">{{ __('sale.return_to') }}</span><br>
                        <span>{{ $party->company_name }}<br>
                        {{ $sale->party->billing_address }}</span>
                        {{-- Party Tax/GST Number --}}
                        @include('print.common.party-tax-details', ['model' => $sale])
                    </td>
                </tr>
            </table>
        ...
        <table class="table-bordered custom-table table-compact" id="item-table">
            ...
            <tfoot>
                ...
                @if(app('company')['is_enable_secondary_currency'])
                    <tr>
                        <td colspan="2" class="text-end fw-bold">{{ __('currency.converted_to').'-'.$sale->currency->code }}</td>
                ...
                <tr>
                    ...
                    <td colspan="1" class="text-end">{{$formatNumber->formatWithPrecision($sale->grand_total - $sale->paid_amount)}}</td>
                </tr>
