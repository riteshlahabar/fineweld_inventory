    ...
    <div class="invoice-container">
        ...
        <div class="invoice">
            <table class="header">
                <tr>
                    ...
                    <td class="bill-info">
                        <span class="bill-number">{{ __('sale.quotation.quotation') }} #: {{ $quotation->quotation_code }}</span><br>
                        <span class="cu-fs-16">{{ __('app.date') }}: {{ $quotation->formatted_quotation_date  }}</span><br>
                        <span class="cu-fs-16">{{ __('app.time') }}: {{ $quotation->format_created_time }}</span><br>
            ...
            <table class="addresses">
                <tr>
                    <td class="address">
                        ...
                        <span>{{ $party->company_name }}<br>
                        {{ $quotation->party->billing_address }}</span>
                    ...
                    <td class="address">
                        ...
                        <span>{{ $quotation->party->shipping_address }}</span>
                    </td>
                </tr>
            </table>
        ...
        <table class="table-bordered custom-table table-compact" id="item-table">
            ...
            <tbody>
                ...
                <tr class="fw-bold">
                    ...
                    <td class="text-end">
                            {{ $formatNumber->formatWithPrecision($quotation->itemTransaction->sum('quantity')) }}
                    </td>
                    <td></td>
                    @if(app('company')['show_discount'])
                    <td class="text-end">
                            {{ $formatNumber->formatWithPrecision($quotation->itemTransaction->sum('discount_amount')) }}
                    </td>
                    @endif
                    @if(app('company')['tax_type'] != 'no-tax')
                    <td class="text-end">
                            {{ $formatNumber->formatWithPrecision($quotation->itemTransaction->sum('tax_amount')) }}
                    </td>
                    @endif
                    <td class="text-end">
                            {{ $formatNumber->formatWithPrecision($quotation->itemTransaction->sum('total')) }}
                    </td>
                </tr>
            </tbody>
            <tfoot>
                ...
                <tr>
                    <td colspan="{{ $noteColumns }}" rowspan="{{$noteRosSpan}}" class="tfoot-first-td">
                        <span class="invoice-note">Note:<br></span>{{ $quotation->note }}
                    </td>
                ...
                <tr>
                    ...
                    <td colspan="1" class="text-end">{{ $formatNumber->formatWithPrecision($quotation->round_off) }}</td>
                </tr>
                <tr>
                    <td colspan="{{ $noteColumns }}" rowspan="{{ $amountInWordsRowSpan }}" class="tfoot-first-td">
                        <b>{{ __('app.amount_in_words') }}:<br></b>
                        {{ ucwords($formatNumber->spell($quotation->grand_total)) }}
                    </td>
                    <td colspan="2" class="text-end fw-bold">{{ __('app.grand_total') }}</td>
                    <td colspan="1" class="text-end">{{ $formatNumber->formatWithPrecision($quotation->grand_total) }}</td>
                </tr>
                @if(app('company')['is_enable_secondary_currency'])
                    <tr>
                        <td colspan="2" class="text-end fw-bold">{{ __('currency.converted_to').'-'.$quotation->currency->code }}</td>
                        <td colspan="1" class="text-end">{{$formatNumber->formatWithPrecision($quotation->grand_total * $quotation->exchange_rate)}}</td>
                    </tr>
                @endif
                ...
                <tr>
                    @php
                        $savedAmount = $quotation->itemTransaction->sum(function ($transaction) {
                ...
                <tr>
                    @php
                        $partyTotalDue = $quotation->party->getPartyTotalDueBalance();
                    ...
                    @endphp
                    <td colspan="2" class="text-end fw-bold">{{ __('app.previous_due_balance') }}</td>
        ...
        <table class="table table-bordered custom-table tax-breakdown table-compact">
            ...
            <tbody>
                ...
                if(app('company')['tax_type'] == 'tax'){
                    $taxSummary = $quotation->itemTransaction
                        ->groupBy('tax_id')
                        ->map(function ($group) {
                ...
                @foreach($taxSummary as $summary)
                    ...
                    <tr>
                        ...
                        @php
                            ...
                            if(empty($quotation->state_id) || app('company')['state_id'] == $quotation->state_id){
