        ...
        <div class="invoice">
            <table class="header">
                <tr>
                    ...
                    <td class="bill-info">
                        <span class="bill-number">#{{ $purchase->return_code }}</span><br>
                        <span class="cu-fs-16">{{ __('purchase.return.date') }}: {{ $purchase->formatted_purchase_date  }}</span><br>
                        <span class="cu-fs-16">{{ __('app.time') }}: {{ $purchase->format_created_time  }}</span><br>
                        @if($purchase->reference_no)
                        <span class="cu-fs-16">{{ __('purchase.purchase_bill_number') }}: {{ $purchase->reference_no  }}</span><br>
                        @endif
                    </td>
                </tr>
            </table>
            <table class="addresses">
                <tr>
                    <td class="address">
                        <span class="fw-bold cu-fs-18">{{ __('purchase.return_to') }}</span><br>
        ...
        <table class="table-bordered custom-table table-compact" id="item-table">
            ...
            <tbody>
                ...
                <tr class="fw-bold">
                    ...
                    <td class="text-end">
                            {{ $formatNumber->formatWithPrecision($purchase->itemTransaction->sum('quantity')) }}
                    </td>
                    <td></td>
                    @if(app('company')['show_discount'])
                    <td class="text-end">
                            {{ $formatNumber->formatWithPrecision($purchase->itemTransaction->sum('discount_amount')) }}
                    </td>
                    @endif
                    @if(app('company')['tax_type'] != 'no-tax')
                    <td class="text-end">
            ...
            <tfoot>
                ...
                @php
                    ...
                    $amountInWordsRowSpan = 3 + app('company')['show_party_due_payment']+app('company')['is_enable_secondary_currency'];
                @endphp
                <tr>
                    <td colspan="{{ $noteColumns }}" rowspan="{{$noteRosSpan}}" class="tfoot-first-td">
                        <span class="invoice-note">Note:<br></span>{{ $purchase->note }}
                    </td>
                ...
                <tr>
                    <td colspan="{{ $noteColumns }}" rowspan="{{ $amountInWordsRowSpan }}" class="tfoot-first-td">
                        ...
                        {{ ucwords($formatNumber->spell($purchase->grand_total)) }}
                    </td>
                    <td colspan="2" class="text-end fw-bold">{{ __('app.grand_total') }}</td>
                    <td colspan="1" class="text-end">{{ $formatNumber->formatWithPrecision($purchase->grand_total) }}</td>
                </tr>
                @if(app('company')['is_enable_secondary_currency'])
                    <tr>
                        <td colspan="2" class="text-end fw-bold">{{ __('currency.converted_to').'-'.$purchase->currency->code }}</td>
                        <td colspan="1" class="text-end">{{$formatNumber->formatWithPrecision($purchase->grand_total * $purchase->exchange_rate)}}</td>
                    </tr>
                @endif
                <tr>
                    <td colspan="2" class="text-end fw-bold">{{ __('payment.paid_amount') }}</td>
                    <td colspan="1" class="text-end">{{$formatNumber->formatWithPrecision($purchase->paid_amount)}}</td>
                </tr>
                <tr>
                ...
                @foreach($taxSummary as $summary)
                    ...
                    <tr>
                        ...
                        @php
                            ...
                            if(empty($purchase->state_id) || app('company')['state_id'] == $purchase->state_id){
